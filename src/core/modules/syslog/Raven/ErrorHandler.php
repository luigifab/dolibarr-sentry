...
class Raven_ErrorHandler
{
    ...
    {
        if ((stripos($e->getMessage(), 'Undefined property') === 0) ||
            (stripos($e->getMessage(), 'Undefined array key') === 0) ||
            (stripos($e->getMessage(), 'Attempt to read property') === 0) ||
            (stripos($e->getMessage(), 'Undefined variable') === 0) ||
            (stripos($e->getMessage(), 'Undefined index') === 0) ||
            (stripos($e->getMessage(), 'Trying to access array offset') === 0) ||
            (stripos($e->getMessage(), 'Trying to get property') === 0)
           ) {
            // ignore Undefined property... car Undefined property, c'est la vie pour Dolibarr
            return;
        }
